...
function boolEnvOrAutoByKey(name, fallbackKeyName) {
  if (process.env[name] == null || process.env[name] === "") {
    return Boolean(String(process.env[fallbackKeyName] || "").trim());
  }
  return (
    String(process.env[name]).toLowerCase() === "true" ||
    process.env[name] === "1"
...
module.exports = {
  ...
  medicalBackend: {
    baseUrl: normalizeMedicalBackendBaseUrl(
      process.env.MEDICAL_BACKEND_BASE_URL,
    ),
    ...
    healthTimeoutMs: Number(
      process.env.MEDICAL_BACKEND_HEALTH_TIMEOUT_MS || 10000,
    ),
  ...
  sentry: {
    ...
    profileSessionSampleRate: numberEnv(
